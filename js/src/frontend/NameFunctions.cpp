  ...
  public:
    ...
    bool resolve(ParseNode* const cur, HandleAtom prefixArg = nullptr) {
        ...
        switch (cur->getKind()) {
          ...
          // ParseNodeKind::PNK_COMPREHENSIONFOR for comprehensions, ParseNodeKind::PNK_LEXICALSCOPE for
          // legacy comprehensions.  Probably this should be a non-list
          // eventually.
          case ParseNodeKind::PNK_ARRAYCOMP:
            MOZ_ASSERT(cur->isArity(PN_LIST));
            MOZ_ASSERT(cur->pn_count == 1);
            MOZ_ASSERT(cur->pn_head->isKind(ParseNodeKind::PNK_LEXICALSCOPE) ||
                       cur->pn_head->isKind(ParseNodeKind::PNK_COMPREHENSIONFOR));
          ...
          case ParseNodeKind::PNK_IMPORT_SPEC_LIST: {
            ...
            for (; item; item = item->pn_next) {
                MOZ_ASSERT(item->isKind(isImport ? ParseNodeKind::PNK_IMPORT_SPEC : ParseNodeKind::PNK_EXPORT_SPEC));
          ...
          case ParseNodeKind::PNK_CATCHLIST: {
            ...
            for (ParseNode* catchNode = cur->pn_head; catchNode; catchNode = catchNode->pn_next) {
                MOZ_ASSERT(catchNode->isKind(ParseNodeKind::PNK_LEXICALSCOPE));
                MOZ_ASSERT(catchNode->scopeBody()->isKind(ParseNodeKind::PNK_CATCH));

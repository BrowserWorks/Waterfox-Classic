...
PushNodeChildren(ParseNode* pn, NodeStack* stack)
{
    switch (pn->getKind()) {
      ...
      // Default clauses are ParseNodeKind::PNK_CASE but do not have case expressions.
      ...
      case ParseNodeKind::PNK_EXPORT_FROM: {
        MOZ_ASSERT(pn->isArity(PN_BINARY));
        MOZ_ASSERT_IF(pn->isKind(ParseNodeKind::PNK_IMPORT), pn->pn_left->isKind(ParseNodeKind::PNK_IMPORT_SPEC_LIST));
        MOZ_ASSERT_IF(pn->isKind(ParseNodeKind::PNK_EXPORT_FROM), pn->pn_left->isKind(ParseNodeKind::PNK_EXPORT_SPEC_LIST));
      ...
      // ParseNodeKind::PNK_COMPREHENSIONFOR for comprehensions, ParseNodeKind::PNK_LEXICALSCOPE for legacy
      // comprehensions.  Probably this should be a non-list eventually.
      case ParseNodeKind::PNK_ARRAYCOMP: {
#ifdef DEBUG
        MOZ_ASSERT(pn->isKind(ParseNodeKind::PNK_ARRAYCOMP));
        MOZ_ASSERT(pn->isArity(PN_LIST));
        MOZ_ASSERT(pn->pn_count == 1);
        MOZ_ASSERT(pn->pn_head->isKind(ParseNodeKind::PNK_LEXICALSCOPE) ||
                   pn->pn_head->isKind(ParseNodeKind::PNK_COMPREHENSIONFOR));
#endif
        return PushListNodeChildren(pn, stack);
      }

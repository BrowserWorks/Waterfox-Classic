...
PushNodeChildren(ParseNode* pn, NodeStack* stack)
{
    switch (pn->getKind()) {
      ...
      // ParseNodeKind::ComprehensionFor for comprehensions, ParseNodeKind::LexicalScope for legacy
      // comprehensions.  Probably this should be a non-list eventually.
      case ParseNodeKind::ArrayComp: {
#ifdef DEBUG
        MOZ_ASSERT(pn->isKind(ParseNodeKind::ArrayComp));
        MOZ_ASSERT(pn->isArity(PN_LIST));
        MOZ_ASSERT(pn->pn_count == 1);
        MOZ_ASSERT(pn->pn_head->isKind(ParseNodeKind::LexicalScope) ||
                   pn->pn_head->isKind(ParseNodeKind::ComprehensionFor));
#endif
        return PushListNodeChildren(pn, stack);
      }

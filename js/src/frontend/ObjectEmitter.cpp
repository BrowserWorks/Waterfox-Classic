...
#include "jsopcode.h"                  // IsHiddenInitOp
#include "jscntxt.h"                   // JSContext
#include "vm/NativeObject.h"           // NativeDefineProperty
#include "vm/ObjectGroup.h"            // TenuredObject
#include "vm/Opcodes.h"                // JSOP_*
#include "vm/Runtime.h"                // JSAtomState (cx->names())

#include "jsgc.h"               // GetGCObjectKind
...
bool PropertyEmitter::emitInit(JSOp op, JS::Handle<JSAtom*> key) {
  ...
  if (obj_) {
    ...
    JS::Rooted<jsid> id(bce_->cx, AtomToId(key));
    if (!NativeDefineProperty(bce_->cx, obj_, id, UndefinedHandleValue,

  ...
  public:
    ...
    union {
        ...
        struct {                        /* two kids if binary */
            ...
            ParseNode*  right;
            union {
                unsigned iflags;        /* JSITER_* flags for ParseNodeKind::PNK_{COMPREHENSION,}FOR node */
...
#define PNX_NONCONST    0x04            /* initialiser has non-constants */
    ...
    ParseNode* generatorExpr() const {
        MOZ_ASSERT(isKind(ParseNodeKind::PNK_GENEXP));

        ParseNode* callee = this->pn_head;
        MOZ_ASSERT(callee->isKind(ParseNodeKind::PNK_FUNCTION));

        ParseNode* paramsBody = callee->pn_body;
        MOZ_ASSERT(paramsBody->isKind(ParseNodeKind::PNK_PARAMSBODY));

        ParseNode* body = paramsBody->last();
        MOZ_ASSERT(body->isKind(ParseNodeKind::PNK_STATEMENTLIST));
        MOZ_ASSERT(body->last()->isKind(ParseNodeKind::PNK_LEXICALSCOPE) ||
                   body->last()->isKind(ParseNodeKind::PNK_COMPREHENSIONFOR));
        return body->last();
    }

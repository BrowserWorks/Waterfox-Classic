...
Parser<ParseHandler, CharT>::matchOrInsertSemicolon()
{
    ...
    if (tt != TokenKind::TOK_EOF &&
        ...
        if (!yieldExpressionsSupported() &&
            anyChars.currentToken().type == TokenKind::TOK_YIELD) {
...
Parser<ParseHandler, CharT>::forStatement(YieldHandling yieldHandling)
{
    ...
    if (allowsForEachIn()) {
        bool matched;
        if (!tokenStream.matchToken(&matched, TokenKind::TOK_EACH))
            return null();
        if (matched) {
...
Parser<ParseHandler, CharT>::tryStatement(YieldHandling yieldHandling)
{
     ...
     * kid1 is the lvalue (possible identifier, TokenKind::TOK_LB, or TokenKind::TOK_LC)
...
Parser<ParseHandler, CharT>::comprehensionFor(GeneratorKind comprehensionKind)
{
    ...
    MUST_MATCH_TOKEN(TokenKind::TOK_LP, JSMSG_PAREN_AFTER_FOR);
    ...
    if (!lhs)
        return null();
    bool matched;
    if (!tokenStream.matchToken(&matched, TokenKind::TOK_OF))
        return null();
    if (!matched) {
    ...
    MUST_MATCH_TOKEN_MOD(TokenKind::TOK_RP, TokenStream::Operand, JSMSG_PAREN_AFTER_FOR_OF_ITERABLE);
...
{
    MOZ_ASSERT(anyChars.isCurrentTokenType(TokenKind::TOK_IF));
...
Parser<ParseHandler, CharT>::comprehensionTail(GeneratorKind comprehensionKind)
{
    ...
    bool matched;
    if (!tokenStream.matchToken(&matched, TokenKind::TOK_FOR, TokenStream::Operand))
        return null();
    if (matched)
...
// `for`. The caller is responsible for matching the ending TokenKind::TOK_RP or
// TokenKind::TOK_RB.
template <class ParseHandler, typename CharT>
typename ParseHandler::Node
...
Parser<ParseHandler, CharT>::arrayComprehension(uint32_t begin)
{
    ...
    MUST_MATCH_TOKEN_MOD(TokenKind::TOK_RB, TokenStream::Operand, JSMSG_BRACKET_AFTER_ARRAY_COMPREHENSION);
        ...
        if (tt == TokenKind::TOK_TRIPLEDOT) {
            ...
            if (propType == PropertyType::Normal) {
                ...
                if (propAtom == context->names().proto) {
                    ...
                    // Note: this occurs *only* if we observe TokenKind::TOK_COLON!
